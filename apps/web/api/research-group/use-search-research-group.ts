...
async function searchResearchGroup(query: string): Promise<GrupoPesquisa[]> {
    const apiURL = process.env.NEXT_PUBLIC_API_URL || ''

    const { data } = await api(apiURL, false).post<GrupoPesquisa[]>(`/researchgroup/search?data=${query}`)
